...
+ js 함수는 값의 성질을 갖는 객체인 ⭐`일급 객체`이다 (함수를 값처럼 자유롭게 사용 가능)
___
<br/><br/>
...
// ✔️함수 호출 시 매개변수의 개수만큼 인수를 전달하지 않아도 에러발생❌
...
// ✔️매개변수보다 인수가 더 많은 경우 초과된 인수는 무시
console.log(add(2,5,7)); 
```
...
- 위와 같이 함수 내부에서 적절한 인수가 전달되었는지 확인하더라도 부적절한 호출을 사전에 방지할 수 없고 에러는 런타임에 발생
  → ✔️타입 스크립트 사용✔️해 컴파일 시점에 부적절한 호출 방지하여 해결 가능 
- arguments 객체 통해 인수 개수 확인도 가능
```
2️⃣  인수가 전달되지 않은 경우 ✔️`단축평가를 사용`해 매개변수에 기본값을 할당하는 방법 
```js
function add(a,b,c){
    a = a || 0;
    b = b || 0;
...
+ 위처럼 함수가 외부 상태(위 코드에서 객체를 할당한 person)를 변경했을 때 상태 변경 추적하기 어려워지는 점 개선하는 방법<br/>
  ...
  2. 객체를 불변 객체로 만들어 사용<br/>
    객체의 복사본 만들어 원시값처럼 변경 불가능한 값으로 동작하게 만들기 (객체의 상태 변경 필요한 경우 ✔️`깊은 복사`이용)<br/>
___
<br/><br/>
# 12.7 다양한 함수의 형태 
## 12.7.1 즉시 실행 함수(ㄴ잘안씀)
...
```js
var count = 0; // ✔️ㄴ count 자체 값은 바뀌지 않는다
...
```js
var count = 0; ✔️ㄴ count 자체 값이 바뀐다 
