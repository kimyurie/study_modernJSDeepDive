...
let foo;
```
+ let 키워드로 선언한 변수는 `선언 단계와 초기화 단계가 분리`되어 실행 (var은 같이 실행됨)<br/>
...
+ 일반적으로 상수 이름은 대문자로 선언하며 여러 단어로 이루어진 경우 스네이크 케이스(_)로 표현하는 게 일반적이다 
___
## 15.3.4 const 키워드와 객체
+ const 키워드로 선언한 변수에 객체를 할당한 경우에는 값을 변경할 수 있다(객체는 재할당 없이 직접 변경 가능하기 때문)
```js
const person = {
    name : 'Lee'
};
